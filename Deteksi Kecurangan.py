...
# Timer untuk deteksi wajah menghilang
last_seen = time.time()
...
while True:
    ...
    if len(faces) > 1:
        ...
        with open("cheating_log.txt", "a") as log:
            log.write(msg + "\n")
        cv2.imwrite(f"screenshots/cheating_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg", frame)
    ...
    if len(faces) == 0:
        if face_missing_start is None:
            ...
            with open("cheating_log.txt", "a") as log:
                log.write(msg + "\n")
            cv2.imwrite(f"screenshots/missing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg", frame)
